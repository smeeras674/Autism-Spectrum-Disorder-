...
# proposed update is done at line 57

def PROPOSED(Prey, fobj, lb, ub, Max_iter):  # Improved Marine Predators Algorithm (MPA)
    ...
    while Iter < Max_iter:
        ...
        for i in range(Prey.shape[1 - 1]):
            Flag4ub = Prey[i, :] > ub[i]
            Flag4lb = Prey[i, :] < lb[i]
            Prey[i, :] = (np.multiply(Prey[i, :], (~ (Flag4ub + Flag4lb)))) + np.multiply(ub[i], Flag4ub) + np.multiply(
                lb[i], Flag4lb)
        ...
        for i in range(Prey.shape[1 - 1]):
            for j in range(Prey.shape[2 - 1]):
                # update using proposed position
                # R = np.random.rand()
                R = (np.min(fitness)) / ((np.max(fitness) / np.min(fitness)) / (np.min(fitness) / fitness[i]))
                # ------------------ Phase 1 (Eq.12) -------------------
                if Iter < Max_iter / 3:
        ...
        for i in range(Prey.shape[1 - 1]):
            Flag4ub = Prey[i, :] > ub
            Flag4lb = Prey[i, :] < lb
            Prey[i, :] = (np.multiply(Prey[i, :], (~ (Flag4ub[i] + Flag4lb[i])))) + np.multiply(ub[i], Flag4ub[
                i]) + np.multiply(lb[i], Flag4lb[i])
            fitness[i] = fobj(Prey[i, :])
            if fitness[i] < Top_predator_fit:
                Top_predator_fit = fitness[i]
                Top_predator_pos = Prey[i, :]
        ...
        if np.random.rand() < FADs:
            ...
            Prey = Prey + CF * (
                np.multiply((Xmin[:10, :] + np.multiply(np.random.rand(SearchAgents_no, dim),
                                                        (Xmax[:SearchAgents_no, :] - Xmin[:SearchAgents_no, :]))), U))
        else:
            r = np.random.rand()
            Rs = Prey.shape[1 - 1]
