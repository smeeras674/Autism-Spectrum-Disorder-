...
def Model_ANN(train_data, train_target, test_data, test_target):
    ...
    for i in range(train_target.shape[1]):
        out, model2 = train_ann(train_data, train_target[:, i], test_data, 10)  # DNN
...
def train_ann(data, labels, test_data, neu):
    ...
    for i in range(len(test_data)):
        # pred[i] = simple_network.run(data[i, :])
        pred[i] = simple_network.run(test_data[i, -1])
        if pred[i] > 0.5:
            pred[i] = 1
        else:
            pred[i] = 0
